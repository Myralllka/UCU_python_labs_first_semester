...
def semester_calendar(output_type, year, first_month, last_month):
    ...
    print calendar of needed year in txt or html type
    ...
    result = ""
    for each in range(first_month, last_month + 1):
        if (output_type == "txt"):
            cal = calendar\
                .TextCalendar(calendar.MONDAY)\
                .formatmonth(year, each, 0, 0)
        else:
            cal = calendar.HTMLCalendar(calendar.MONDAY)
            cal = cal.formatmonth(year, each)
        result += cal
        if not(each == last_month):
            result += "\n"
    return result
...
if __name__ == "__main__":
    ...
    print(semester_calendar("txt", 2018, 11, 12))
